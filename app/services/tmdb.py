...
def _make_bearer_session(token):
    """Bearer token auth — the official TMDB v4 method, bypasses IP blocking."""
...
def _make_key_session():
    """Fallback: API key in URL params."""
    session = requests.Session()
    session.headers.update({
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/122.0.0.0 Safari/537.36',
        'Accept':     'application/json',
...
class TMDBService:
    def __init__(self):
        ...
        # Bearer token takes priority — check env directly AND config
        self.token = (
            os.environ.get('TMDB_READ_TOKEN') or
            current_app.config.get('TMDB_READ_TOKEN', '')
        )

        if self.token:
            self.session    = _make_bearer_session(self.token)
            self.use_bearer = True
            print("TMDB: Using Bearer token auth ✓")
        else:
            self.session    = _make_key_session()
            self.use_bearer = False
            print("TMDB: Using API key auth (Bearer token not found)")

        self.max_pages = 2 if IS_PRODUCTION else 6
    ...
    def _get(self, endpoint, params=None):
        ...
        # Only add api_key to params when NOT using bearer
        ...
        for attempt in range(4):
            try:
                if attempt > 0:
                    time.sleep(min(attempt * 0.8, 3))
                    # Fresh session with new headers on retry
                    if self.use_bearer:
                        self.session = _make_bearer_session(self.token)
                    else:
                        self.session = _make_key_session()
                ...
                if res.status_code == 401:
                    print(f"TMDB 401 Unauthorized — token/key invalid")
                    return {}
                ...
                print(f"TMDB connection error (attempt {attempt+1}): {e}")
            except requests.exceptions.Timeout:
                print(f"TMDB timeout on {endpoint} (attempt {attempt+1})")
            except Exception as e:
                print(f"TMDB error {endpoint} (attempt {attempt+1}): {e}")
